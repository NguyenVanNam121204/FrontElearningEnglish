    ...
    return (
        <div className="matching-question">
            <div className="matching-instructions">
                <p>Kéo thả hoặc click để nối các cặp từ/cụm từ tương ứng</p>
            </div>
            
            <Row className="matching-container">
                <Col md={6}>
                    <div className="matching-column">
                        <h4 className="column-title">Cột trái</h4>
                        {leftOptions.map((option, index) => {
                            ...
                            return (
                                <div
                                    key={option.id}
                                    className={`matching-item left-item ${isSelected ? "selected" : ""} ${matchedRight ? "matched" : ""}`}
                                    onClick={() => {
                                        if (matchedRight) {
                ...
                <Col md={6}>
                    <div className="matching-column">
                        <h4 className="column-title">Cột phải</h4>
                        {rightOptions.map((option, index) => {
                            ...
                            return (
                                <div
                                    key={option.id}
                                    className={`matching-item right-item ${isSelected ? "selected" : ""} ${isMatched ? "matched" : ""}`}
                                    onClick={() => {
                                        if (!isMatched) {
                                ...
                                >
                                    <span className="item-number">{String.fromCharCode(65 + index)}</span>
                                    <span className="item-text">{option.text}</span>
                                </div>
                            );
                        })}
